...
export default function CatalogController({ productsResponseEncoded, attributes, categories, selectedCategories, exchangeState, currency }: {
    ...
    function FiltersDesktop({ className }: { className?: string }) {
        return (
            <div className={`${className} flex flex-wrap items-center`}>
                ...
                <PriceFilter defaultPriceMin={moneySmallToBig(productsResponse.priceMin)} defaultPriceMax={moneySmallToBig(productsResponse.priceMax)}></PriceFilter>
    ...
    function CatalogView() {
        return (
            <Fragment>
                ...
                <FiltersButtonMobile toggle={() => setCurrentViewScreen(CurrentMobileViewScreen.Filters)} className="flex lg:hidden"></FiltersButtonMobile>
                <div className="flex">
    ...
    function MobileScreenViewBase({ title, children }: { title: string, children: React.ReactNode }) {
        return (
            <div className="fixed w-full h-full top-0">
                <div className="flex flex-col h-full w-full bg-white dark:bg-slate-800">
                    <div className="flex">
                        ...
                        <button
                            onClick={() => setCurrentViewScreen(CurrentMobileViewScreen.Catalog)}
                            className="
    ...
    function FiltersView() {
        return (
            <MobileScreenViewBase title="Filters">
                <SortFilter></SortFilter>
                <PriceFilter defaultPriceMin={moneySmallToBig(productsResponse.priceMin) as number} defaultPriceMax={moneySmallToBig(productsResponse.priceMax) as number}></PriceFilter>
                {Object.entries(productsResponse.filters).map(([key, values]) => (
                        <AttributeFilter
                            key={key}
                            values={values}
    ...
    const ConfigToScreen = {
        [CurrentMobileViewScreen.Catalog]: CatalogView,
        [CurrentMobileViewScreen.Filters]: FiltersView,
        [CurrentMobileViewScreen.Menu]: MenuView,
    }

    return (
        { ...ConfigToScreen[currentViewScreen]() }
    )
}
